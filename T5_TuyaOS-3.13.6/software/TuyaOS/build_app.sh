...
FINGERPRINT_FILE=output/.app_build_fingerprint
CUR_FINGERPRINT="${APP_NAME}-${APP_VERSION}-${TARGET_PLATFORM}"
if [ x"clean" != x$USER_CMD ]; then
    mkdir -p $(dirname $FINGERPRINT_FILE)
    if [ ! -f $FINGERPRINT_FILE ] || [ "$(cat $FINGERPRINT_FILE)" != "$CUR_FINGERPRINT" ]; then
        echo "fingerprint changed, run app_static_clean"
        make app_static_clean
        echo "$CUR_FINGERPRINT" > $FINGERPRINT_FILE
    fi
fi

if [ x"clean" = x$USER_CMD ]; then
    make app_clean_by_name APP_NAME=$APP_NAME APP_VER=$USER_SW_VER USER_CMD=$USER_CMD
    rm -rf $FINGERPRINT_FILE
else
    ...
    if [ ! x"$CI_PACKAGE_PATH" = x"" ]; then
        if [ -f ./build/os_cube_api.json ]; then
            make app_cube APP_NAME=$APP_NAME APP_VER=$USER_SW_VER
            if [ -f ./build/app_cube_api.json ]; then
                cp ./build/app_cube_api.json $CI_PACKAGE_PATH
            fi
        fi
    fi
fi
