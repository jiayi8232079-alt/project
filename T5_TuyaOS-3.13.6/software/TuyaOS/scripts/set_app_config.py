...
def set_iotosconfig(make_yaml, osconfig, platform):
    ...
    for block in comp_deps:
        if "Component" in block.keys():
            ...
            ex_config_path = os.path.join("apps/"+APP_NAME, locater,
                                          name, 'APPconfig')
            if os.path.exists(ex_config_path):
                comp_context += f'    rsource "../{config_path}"\n'
    ...
    for block in driver_deps:
        if "Component" in block.keys():
            ...
            ex_config_path = os.path.join("apps/"+APP_NAME, locater,
                                          name, 'APPconfig')
            if os.path.exists(ex_config_path):
                driver_context += f'    rsource "../{config_path}"\n'
            elif os.path.exists(config_path):
                driver_context += f'    rsource "../../../{config_path}"\n'
    if driver_context:
    ...
    if platform:
        ...
        if os.path.exists(config_path):
            vendor_context += f'    rsource "../../../{config_path}"\n'
    if vendor_context:
