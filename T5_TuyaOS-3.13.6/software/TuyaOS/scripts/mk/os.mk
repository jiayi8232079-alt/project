...
os_files:
	...
	@if [ ! x"$(TUYA_SDK_NO_INC_EXPORT)" = x"" ]; then cp -a --parents $(TUYA_SDK_NO_INC_EXPORT) -t ./include; fi
	...
	@if [ -f $(OS_CUBE_JSON) ]; then cp $(OS_CUBE_JSON) $(OS_FILES_DIR)/build; fi
	@cp -fr ./build/build_param.template $(OS_FILES_DIR)/build
	@rm -fr ./include/vendor/ -rf
	@rm -fr ./include/apps/ -rf
