    ...
    for block in sdk_deps:
        if "Component" in block.keys():
            ...
            if os.path.exists(config_path):
                sdk_context += f'    rsource "../{config_path}"\n'
    if sdk_context:
        sdk_context = SDK_CONTEXT_PREFIX + sdk_context + SUFFIX

    platform_context = ""
    if platform:
        config_path = "vendor/" + platform + "/tuyaos/IoTOSconfig"
        if os.path.exists(config_path):
            platform_context += f'    rsource "../{config_path}"\n'

    context = 'mainmenu "Tuya IoT SDK Configuration"\n'
    context += sdk_context
    if platform_context:
        platform_context = VENDOR_CONTEXT_PREFIX + platform_context + SUFFIX
