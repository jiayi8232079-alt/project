...
topics+=(`grep -o '/tuyaos_jiajia/src/miscs/\(.*\)' symbols.all | cut -d/ -f5  | sort | uniq` )
topics+=(`grep -o '/tuyaos_jiajia/src/drivers/\(.*\)' symbols.all | cut -d/ -f5  | sort | uniq` )
topics+=("tuyaos_jiajia" "wukong"  "vendor")
