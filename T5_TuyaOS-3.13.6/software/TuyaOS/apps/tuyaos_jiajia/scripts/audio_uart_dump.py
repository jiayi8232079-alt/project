...
class SerialTerminal:
    def __init__(self):
        ...
        self.dump_files = {
            ...
            '4': {'file': None, 'name': 'dump_vad.pcm', 'active': False, 'length': 0}
        }
...
def print_help():
    ...
    print("  stop                  - 停止录音")
    print("  reset                 - 重置录音")
    print("  dump <0-4>            - 转储音频通道 (0=mic, 1=ref, 2=aec, 3=asr, 4=vad)")
    print("  netdump <0-4>         - 网络转储音频通道")
    print("  realtime start [batch] [framesize]")
    print("                        - 启动实时流 (默认: batch=50, framesize=640)")
    print("  realtime stop         - 停止实时流")
    print("  bg <0-4> [freq]       - 播放测试音 (0=白噪声, 1=单频, 2=扫频, 3=离散扫频, 4=最小信号)")
    print("  volume <0-100>        - 设置音量")
    print("  micgain <0-100>       - 设置麦克风增益 (默认=70)")
    print("  alg set <para> <value>")
    print("                        - 设置算法参数")
    print("  alg get <para>        - 获取算法参数")
    print("  alg dump              - 转储所有算法参数")
    print("  help                  - 显示此帮助")
    print("  quit                  - 退出程序")
    print("")
...
def main():
    ...
    try:
        while True:
            ...
            # 处理dump命令
            elif user_input in ['dump 0', 'dump 1', 'dump 2', 'dump 3', 'dump 4']:
            ...
            elif user_input in ['netdump 0', 'netdump 1', 'netdump 2', 'netdump 3', 'netdump 4']:
                terminal.send_command(user_input)
            # 处理 realtime 命令
            elif user_input.startswith('realtime '):
                parts = user_input.split()
                if len(parts) >= 2:
                    if parts[1] == 'start':
                        cmd = "realtime start"
                        ...
                        terminal.send_command("realtime stop")
                    else:
                        print("命令格式: realtime start [batch] [framesize] 或 realtime stop")
                else:
                    print("命令格式: realtime start [batch] [framesize] 或 realtime stop")
