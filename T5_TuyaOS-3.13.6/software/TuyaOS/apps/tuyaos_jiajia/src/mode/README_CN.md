...
各模式（长按 / 单次 / 唤醒 / 自由 / P2P / 翻译）的启用与板型、功能开关等均在应用配置中完成。配置入口为 `make app_menuconfig APP_NAME=tuyaos_jiajia`，配置会生成到应用根目录下的 **build** 目录（如 `APPConfig` 等），修改后需执行 `make app_config` 生成头文件。具体说明见：
...
3. **生成配置**：保存后执行 `make app_config APP_NAME=tuyaos_jiajia` 生成头文件。
