...
This guide introduces the **Wukong AI Hardware Development Framework** and this sample (**tuyaos_jiajia**): concepts, environment setup, product creation, obtaining the framework, configuration, build, and flash.
...
3. The IDE will download the framework; the tree will include a Demo app (e.g. **tuyaos_jiajia** or similar).
...
1. **Configure the app** (board, features)  
   In the IDE, under `software > TuyaOS > apps`, find the Demo (e.g. **tuyaos_jiajia**). You can run menuconfig from the command line at the SDK root:
   ```bash
...
3. **Build**  
   ...
   - **Output**: After a successful build, the QIO image is under `software/TuyaOS/apps/tuyaos_jiajia/output/<version>/`, e.g. `tuyaos_jiajia_QIO_<version>.bin`.
...
1. In Tuya Wind IDE, locate the built **QIO image** (e.g. `software > TuyaOS > apps > tuyaos_jiajia > output/<version> > tuyaos_jiajia_QIO_<version>.bin`). Right‑click it and choose **Flash App**.
