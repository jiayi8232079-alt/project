...
***********************function define**********************
***********************************************************/
STATIC VOID_T app_fs_init(VOID_T)
{
...
STATIC VOID_T app_mp3_decode_init(VOID_T)
{
    ...
    if (sg_mp3_ctx.decode_hdl == NULL) {
        ...
        TAL_PR_ERR("MP3Decoder init failed!");
        return;
    }
...
STATIC VOID_T app_speaker_init(VOID_T)
{
...
STATIC VOID_T app_speaker_play(VOID_T)
{
    int rt = 0;
    ...
    if (sg_mp3_ctx.decode_hdl == NULL || sg_mp3_ctx.read_buf == NULL || sg_mp3_ctx.pcm_buf == NULL) {
        TAL_PR_ERR("MP3Decoder init fail!");
        return;
    }
    ...
    if (is_exist == FALSE) {
        TAL_PR_ERR("mp3 file not exist!");
        return;
    }

    TUYA_FILE mp3_file = tkl_fopen(mp3_file_path, "r");
    if (NULL == mp3_file) {
        TAL_PR_ERR("open mp3 file %s failed!", mp3_file_path);
        return;
    }
#endif
    ...
    do {
            ...
            if (read_size_remain > 0) {
                memcpy(sg_mp3_ctx.read_buf + sg_mp3_ctx.read_size, MP3_FILE_ARRAY + sg_mp3_ctx.mp3_offset, read_size_remain);
            ...
            read_size_remain = MAINBUF_SIZE - sg_mp3_ctx.read_size;
            INT_T fs_read_len = tkl_fread(sg_mp3_ctx.read_buf + sg_mp3_ctx.read_size, read_size_remain, mp3_file);
            if (fs_read_len <= 0) {
                if (decode_size_remain == 0) { // 最后一帧数据解码播放完毕
                    TAL_PR_NOTICE("mp3 play finish!");
                    break;
...
__MP3_DECODE:
            ...
            if (rt != ERR_MP3_NONE) {
                TAL_PR_ERR("MP3Decode failed, code is %d", rt);
                break;
            }
...
STATIC VOID_T app_speaker_thread(VOID_T *arg)
{
    app_fs_init();
    app_mp3_decode_init();
    app_speaker_init();
...
VOID_T example_speaker_play(VOID_T)
{
