...
当调用 `example_os_wifi_ap` 示例时，会先创建一个 AP 热点，默认热点名称（SSID）为 "my-wifi"，热点密码为"12345678"。热点创建成功后会进行 UDP 广播。

## 流程介绍

`example_os_wifi_ap` 示例的流程为：**WiFi 初始化 -> WiFi 模式设置为 AP 模式 -> 启动 AP -> 每 5s 进行一次 UDP 广播**。

在 `example_os_wifi_ap` 示例中最为重要的代码片段便是： `WF_AP_CFG_IF_S` 结构体了，下面便是对于 `WF_AP_CFG_IF_S` 结构体的一个详细介绍：
...
当 `example_os_wifi_ap` 示例开始执行后，可以使用电脑连接开发板创建 AP 热点，然后通过网络调试工具查看是否可以接收到 UDP 广播包。

下面是开始执行 `example_os_wifi_ap` ，电脑连接到 AP 热点的日志信息：

```
[01-01 00:04:50 ty N][example_os_wifi_ap.c:86] ------ wifi ap example start ------
...
[01-01 00:04:52 ty D][example_os_wifi_ap.c:124] ip_addr:-1062731397
[01-01 00:04:55 ty D][lr:0x88651] feed watchdog
[01-01 00:04:57 ty D][example_os_wifi_ap.c:64] send data:hello world
temp_code:59 - adc_code:281 - adc_trend:[19]:290->[20]:280
[01-01 00:05:02 ty D][example_os_wifi_ap.c:64] send data:hello world
[01-01 00:05:07 ty D][example_os_wifi_ap.c:64] send data:hello world
[01-01 00:05:12 ty D][example_os_wifi_ap.c:64] send data:hello world
[01-01 00:05:15 ty D][lr:0x88651] feed watchdog
[01-01 00:05:17 ty D][example_os_wifi_ap.c:64] send data:hello world
[01-01 00:05:22 ty D][example_os_wifi_ap.c:64] send data:hello world
[01-01 00:05:27 ty D][example_os_wifi_ap.c:64] send data:hello world
...
sta_mgmt_add_key
ctrl_port_hdl:1
[01-01 00:05:32 ty D][example_os_wifi_ap.c:64] send data:hello world
```
