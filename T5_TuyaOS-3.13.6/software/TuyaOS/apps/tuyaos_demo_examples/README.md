...
3. 在该实例中默认使用 `TUYA_UART_NUM_0` 作为命令输入引脚，命令执行的结果通过日志打印口进行输出。
...
固件烧录成功，使用串口工具将电脑和模组进行连接，打开 `TUYA_UART_NUM_0` ，重启芯片   `TUYA_UART_NUM_0`会出现由符号组成的 TUYA OS 的图像（如下图），则说明固件烧录成功。
