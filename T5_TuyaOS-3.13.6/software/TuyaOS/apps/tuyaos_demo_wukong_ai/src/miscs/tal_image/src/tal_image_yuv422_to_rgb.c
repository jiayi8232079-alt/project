...
#if defined(TUYA_DMA2D_SHARE) && (TUYA_DMA2D_SHARE == 1)
...
***********************************************************/
#if defined(TUYA_DMA2D_SHARE) && (TUYA_DMA2D_SHARE == 1)
static BOOL_T sg_is_dma2d_init = FALSE;
static MUTEX_HANDLE sg_dma2d_mutex = NULL;
#endif
...
static OPERATE_RET __dma2d_convert_yuv422_to_rgb565(TAL_IMAGE_YUV422_TO_RGB_T *conv_cfg)
{
    ...
    if (FALSE == sg_is_dma2d_init)
    {
        TUYA_CALL_ERR_RETURN(tuya_dma2d_init());
        TUYA_CALL_ERR_RETURN(tal_mutex_create_init(&sg_dma2d_mutex));
        sg_is_dma2d_init = TRUE;
    }

    tal_mutex_lock(sg_dma2d_mutex);
    ...
    TUYA_CALL_ERR_RETURN(tuya_dma2d_request(TASK_TYPE_CAMERA_CONVERT, &in_frame, &out_frame));

    tuya_dma2d_wait_finish(TASK_TYPE_CAMERA_CONVERT);

    tal_mutex_unlock(sg_dma2d_mutex);
...
static OPERATE_RET __dma2d_convert_yuv422_to_rgb888(TAL_IMAGE_YUV422_TO_RGB_T *conv_cfg)
{
    ...
    tal_semaphore_wait(TY_DMA2D_CAM_CONVERT_BUSY);
