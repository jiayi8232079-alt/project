...
#ifndef ENABLE_LVGL_SNAP_CROSS_PCT
#define ENABLE_LVGL_SNAP_CROSS_PCT 0  
#endif
...
static lv_coord_t elastic_diff(lv_obj_t * scroll_obj, lv_coord_t diff, lv_coord_t scroll_start, lv_coord_t scroll_end,
                               lv_dir_t dir);
#if ENABLE_LVGL_SNAP_CROSS_PCT > 0
static int indev_snap_apply_bias_hor(_lv_indev_proc_t * proc, lv_obj_t * scroll_obj, lv_coord_t * diff_x);
static int indev_snap_apply_bias_ver(_lv_indev_proc_t * proc, lv_obj_t * scroll_obj, lv_coord_t * diff_y);
#endif
...
#if ENABLE_LVGL_SNAP_CROSS_PCT > 0
            indev_snap_apply_bias_ver(proc, scroll_obj, &diff_y);
#endif
...
#if ENABLE_LVGL_SNAP_CROSS_PCT > 0
            indev_snap_apply_bias_hor(proc, scroll_obj, &diff_x);
#endif
...
/**
 * @brief Nudge horizontal throw so first snap animation can cross to adjacent cell at ENABLE_LVGL_SNAP_CROSS_PCT drag.
 * @return 1 if diff_x was modified
 */
static int indev_snap_apply_bias_hor(_lv_indev_proc_t * proc, lv_obj_t * scroll_obj, lv_coord_t * diff_x)
{
    if(lv_obj_get_scroll_snap_x(scroll_obj) != LV_SCROLL_SNAP_CENTER) {
        return 0;
    }

    lv_obj_update_layout(scroll_obj);
    lv_coord_t w = lv_obj_get_content_width(scroll_obj);
    if(w <= 0) {
        return 0;
    }

    lv_coord_t thresh = w * ENABLE_LVGL_SNAP_CROSS_PCT / 100;
    if(thresh < 1) {
        thresh = 1;
    }

    lv_coord_t drag = proc->types.pointer.scroll_sum.x;
    if(LV_ABS(drag) < thresh) {
        return 0;
    }

    lv_coord_t snap0 = find_snap_point_x(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, 0);
    lv_coord_t snap_with = find_snap_point_x(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, *diff_x);
    if(snap0 != snap_with) {
        return 0;
    }

    lv_coord_t step = (drag > 0) ? w : -w;
    lv_coord_t d_try = *diff_x + step;
    scroll_limit_diff(proc, &d_try, NULL);
    {
        lv_coord_t snap_try = find_snap_point_x(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, d_try);
        if(snap_try == snap0) {
            return 0;
        }
    }
...
/**
 * @brief Nudge vertical throw so first snap animation can cross to adjacent cell at ENABLE_LVGL_SNAP_CROSS_PCT drag.
 * @return 1 if diff_y was modified
 */
static int indev_snap_apply_bias_ver(_lv_indev_proc_t * proc, lv_obj_t * scroll_obj, lv_coord_t * diff_y)
{
    if(lv_obj_get_scroll_snap_y(scroll_obj) != LV_SCROLL_SNAP_CENTER) {
        return 0;
    }

    lv_obj_update_layout(scroll_obj);
    lv_coord_t h = lv_obj_get_content_height(scroll_obj);
    if(h <= 0) {
        return 0;
    }

    lv_coord_t thresh = h * ENABLE_LVGL_SNAP_CROSS_PCT / 100;
    if(thresh < 1) {
        thresh = 1;
    }

    lv_coord_t drag = proc->types.pointer.scroll_sum.y;
    if(LV_ABS(drag) < thresh) {
        return 0;
    }

    lv_coord_t snap0 = find_snap_point_y(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, 0);
    lv_coord_t snap_with = find_snap_point_y(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, *diff_y);
    if(snap0 != snap_with) {
        return 0;
    }

    lv_coord_t step = (drag > 0) ? h : -h;
    lv_coord_t d_try = *diff_y + step;
    scroll_limit_diff(proc, NULL, &d_try);
    {
        lv_coord_t snap_try = find_snap_point_y(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, d_try);
        if(snap_try == snap0) {
            return 0;
        }
    }
