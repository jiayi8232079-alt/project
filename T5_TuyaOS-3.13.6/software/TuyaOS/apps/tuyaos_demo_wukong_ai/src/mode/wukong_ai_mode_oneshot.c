/**
 * @file wukong_ai_mode_oneshot.c
 * @brief Oneshot (single-press) mode implementation.
 *
 * Implements the oneshot interaction mode, where a single press triggers
 * a turn-based conversation cycle.
 *
...
STATIC AI_CHAT_MODE_PARAM_T s_ai_oneshot = {0};
STATIC AI_CHAT_STATE_E s_ai_cur_state = AI_CHAT_INVALID;

/**
 * @brief Handle ASR result event in oneshot mode.
 *
...
/**
 * @brief Handle text stream event in oneshot mode.
 *
...
/**
 * @brief Handle emotion event in oneshot mode.
 *
...
/**
 * @brief Enter IDLE state in oneshot mode.
 *
...
/**
 * @brief Enter LISTEN state in oneshot mode.
 *
...
/**
 * @brief Enter UPLOAD state in oneshot mode.
 *
...
/**
 * @brief Enter THINK state in oneshot mode.
 *
...
/**
 * @brief Enter SPEAK state in oneshot mode.
 *
...
STATIC OPERATE_RET wukong_ai_oneshot_int_cb(VOID *data, INT_T len)
{
    ...
    CHAT_SUB_STATE_CHANGE(AI_CHAT_SUB_ONESHOT, s_ai_oneshot.state, AI_CHAT_IDLE);

    s_ai_oneshot.wakeup_stat = FALSE;

#ifdef ENABLE_TUYA_UI
    AI_CHAT_SUB_MODE_E trigger_mode = AI_CHAT_SUB_ONESHOT;
...
/**
 * @brief De-initialize oneshot mode.
 *
...
/**
 * @brief Periodic task callback for oneshot mode.
 *
...
/**
 * @brief Event callback for oneshot mode.
 *
...
STATIC OPERATE_RET wukong_ai_oneshot_event_cb(VOID *data, INT_T len)
{
    ...
    switch (event->type) 
    {
        ...
        case WUKONG_AI_EVENT_ASR_ERROR:
        {
            ...
            CHAT_SUB_STATE_CHANGE(AI_CHAT_SUB_ONESHOT, s_ai_oneshot.state, AI_CHAT_LISTEN);
        }
        break;
        ...
        case WUKONG_AI_EVENT_ASR_OK:
        {
            ...
            CHAT_SUB_STATE_CHANGE(AI_CHAT_SUB_ONESHOT, s_ai_oneshot.state, AI_CHAT_THINK);
        ...
        case WUKONG_AI_EVENT_TTS_PRE:
        {
            ...
            CHAT_SUB_STATE_CHANGE(AI_CHAT_SUB_ONESHOT, s_ai_oneshot.state, AI_CHAT_SPEAK);
        }
        break;
...
/**
 * @brief Handle KWS (Keyword Spotting) wakeup in oneshot mode.
 *
 * Not actively used in oneshot mode since KWS is typically disabled,
...
/**
 * @brief Handle VAD (Voice Activity Detection) event in oneshot mode.
 *
 * In oneshot mode, manual VAD start is triggered by key press, and VAD stop
 * may be triggered by key release or an automatic timeout, initiating the UPLOAD state.
 *
...
/**
 * @brief Client run callback for oneshot mode.
 *
...
/**
 * @brief Handle key press event in oneshot mode.
 *
 * Processes physical button presses. In oneshot mode, a single press starts
 * a listening session (turning on manual VAD).
 *
...
/**
 * @brief Handle idle notification in oneshot mode.
 *
...
/**
 * @brief Register the oneshot mode.
 *
...
OPERATE_RET ai_oneshot_register(AI_CHAT_MODE_HANDLE_T **cb)
{
    ...
    s_ai_oneshot_cb.on_init       = wukong_ai_oneshot_int_cb;
    s_ai_oneshot_cb.on_deinit     = wukong_ai_oneshot_deint_cb;
    s_ai_oneshot_cb.on_key        = wukong_ai_oneshot_key_cb;
    s_ai_oneshot_cb.on_task       = wukong_ai_oneshot_task_cb;
    s_ai_oneshot_cb.on_event      = wukong_ai_oneshot_event_cb;
    s_ai_oneshot_cb.on_wakeup     = wukong_ai_oneshot_wakeup;
    s_ai_oneshot_cb.on_vad        = wukong_ai_oneshot_vad;
    s_ai_oneshot_cb.on_client     = wukong_ai_oneshot_client_run;
    s_ai_oneshot_cb.on_notify_idle = wukong_ai_oneshot_notify_idle_cb;
    *cb = &s_ai_oneshot_cb;
    return rt;
}
#endif