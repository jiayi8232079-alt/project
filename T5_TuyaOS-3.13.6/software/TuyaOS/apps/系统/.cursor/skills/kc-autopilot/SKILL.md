...
3. Dispatch first-round tasks to Workers (see §4 task templates)
4. Enter auto-loop (see §3)
```
...
1. **successCriteria check**: If all met → `autopilot_stop({ reason:'goal-achieved' })` + brief report to user
2. **budget check**:
...
4. Reply to Leader via send_to_session when done
```
...
2. **Find issues** → use `task_report(kind:'finding')` / `memory_write` to record
