...
/**
 * 「陪诊助手」13 个 MCP 工具集中注册。
 *
 * 当前所有 tool 返回 **mock 数据**，但都已带：
 * 1. 标准 input schema（涂鸦智能体平台和 MCP Inspector 都能直接看见）
 * 2. `mutating` 标记（写操作前 Agent 需复述确认）
 * 3. `userMessage` 友好话术（失败时给老人念）
 *
 * **接真实数据**只需替换每个 `execute()` 内部实现，调用对应业务 Service。
 * 注入业务 Service 时记得在 `ai-gateway.module.ts` 的 imports 加对应 Module。
 *
  ...
  constructor() {
    ...
    this.register({
      ...
      execute: async (args, ctx) => {
        this.assertOwnTarget(args as { targetId: string }, ctx);
        this.logger.log(
          `[mock] notify_family user=${ctx.userId} target=${ctx.serviceTargetId} severity=${(args as { severity: string }).severity}`,
        );
        return this.success({
          sentCount: 1,
          channels: ['app_push'],
    ...
    this.register({
      ...
      execute: async () => this.success({ recorded: true }),
    });
