 ...
 * @LastEditTime: 2024-07-25 17:50:07
 */
...
typedef struct speexdsp_wrap_config {
    unsigned char pcm_arrange_mode; // 0: mic0 ref0 mic1 ref1; 1: mic0 mic1 ref0 ref1
    struct {
        int sample_rate;
        int mic_channels;
        int ref_channels;
...
#define PCM_ARRANGE_MODE_ALTERNATELY 0
#define PCM_ARRANGE_MODE_CONSECUTIVELY 1

#define SPEEXDSP_WRAP_CONFIG_DEFAULT() {    \
    .pcm_arrange_mode = PCM_ARRANGE_MODE_ALTERNATELY,\
    .audio = {                              \
        .sample_rate = 16000,               \
        .mic_channels = 1,                  \
        .ref_channels = 1,                  \
