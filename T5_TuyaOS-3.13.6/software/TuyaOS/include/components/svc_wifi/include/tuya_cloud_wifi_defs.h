...
#define GWCM_SPCL_MATTER        6   // special For Matter device
#else
#define GWCM_SPCL_HOTRESET      6   // special hot reset 
#endif
...
#define WF_START_OTHER_CFG              5   /*close tuya netcfg, use other netcfg*/
...
#define GWNS_OTHER_CFGED          6   // already get ssid && passwd from other mode.for example:uart、bluetooth、qrcode and so on
#define GWNS_OTHER_UNCFG        7   // no need tuya wifi netcfg, open other netcfg
...
#define GWNS_PROXY_ACTIVED  		11 	// bt actived
#define GWNS_PROXY_CONFIG_WF		12 	//in bt actived,has configed wifi
#define GWNS_MATTER_COMMISSIONED    13 	/*matter commission finished*/
...
#define GWNS_FAST_UNCFG_SMC_AP       4   // current in smart and ap concurrent mode
...
#define GWM_SPECIAL_MATTER_CFG 	4   // Matter config
...
#define STAT_OTHER_UNCFG                    15     //other protocol unconfig mode ,such as matter, homekit , etc.
#define STAT_MATTER_COMMISSIONED            16     //Commissioned in matter
...
typedef struct {
    ...
    CHAR_T *pin;        // strlen(ap_passwd) <= 8,default null
} WF_GW_PROD_INFO_S;
