...
/**
 * @brief Get wifi config state
 *
 * @param[out] nw_stat Network state, see GW_WIFI_NW_STAT_E
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET tuya_wifi_status_set_config(TUYA_WIFI_STATUS_CONFIG wifi_status_config);

#if defined(ENABLE_MATTER) && (ENABLE_MATTER == 1)
/**
 * @brief wifi stat callback function
 * 
 * @param[in] station_stat: wifi station status
 * 
 * @return None
 */

typedef void (*tuya_wifi_stat_callback_fn)(int station_stat);

/**
 * @brief register wifi stat callback function
 * 
 * @param[in] stat_fn: callback function for wifi station status
 * 
 * @return None
 */

OPERATE_RET tuya_wifi_register_status_handler(tuya_wifi_stat_callback_fn stat_fn);


/**
 * @brief update wifi stat to callback function
 * 
 * @param[in] station_stat: new wifi status 
 * 
 * @return None
 */

OPERATE_RET tuya_wifi_update_status_to_handler(WF_STATION_STAT_E station_stat);
#endif
#ifdef __cplusplus
}
#endif
