...
/**
 * @brief Resets the gw from app.
 *
 * This function resets the gw from app. If `isResetFactory` is set to `TRUE`, the gw will be reset to factory settings.
 *
 * @param isResetFactory If `TRUE`, the gw will be reset to factory settings.
 *
 * @return OPERATE_RET Returns `OPRT_OK` on success, otherwise an error code is returned.
 */
OPERATE_RET tuya_iot_wf_gw_remote_reset(BOOL_T isResetFactory);
/**
