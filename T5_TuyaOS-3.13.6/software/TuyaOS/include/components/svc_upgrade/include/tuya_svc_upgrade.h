...
/* Matter OTA�ӿ� */
OPERATE_RET tuya_matter_upgrade_prepare(void);

OPERATE_RET tuya_matter_upgrade_start(UINT32_T ver, CONST CHAR_T *ver_str,
                                      UINT16_T vid, UINT16_T pid,
                                      CONST CHAR_T *sha256, UINT_T file_size,
                                      BOOL_T is_diff_ota, BOOL_T is_atop_ota);

OPERATE_RET tuya_matter_upgrade_finish(void);

OPERATE_RET tuya_matter_upgrade_apply(void);

OPERATE_RET tuya_matter_upgrade_abort(void);

OPERATE_RET tuya_matter_upgrade_data_proc(CONST BYTE_T *p_data_block, UINT_T u_data_block_len);

OPERATE_RET tuya_matter_upgrade_confirm(UINT32_T target_ver, CHAR_T *target_ver_str);
