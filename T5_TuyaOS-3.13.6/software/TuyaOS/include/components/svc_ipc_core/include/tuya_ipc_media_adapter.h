...
typedef enum
{   
    SPEAK_IDLE = 0,     ///<speaker is not using
    SPEAK_BUSY          ///<speaker is used by someone
}SPEAK_STATU_E;
...
//OPERATE_RET tuya_ipc_media_adapter_get_speak_user_handle(INT_T device, INT_T channel, MEDIA_USER_HANDLE *user_handle);

/** @brief get speak status
 * @param[in] device  device number
 * @param[in] channel  channel number
 * @param[out] status speak status
 * @return error code
 * - OPRT_OK  success
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_get_speak_status(INT_T device, INT_T channel, SPEAK_STATU_E *status);
