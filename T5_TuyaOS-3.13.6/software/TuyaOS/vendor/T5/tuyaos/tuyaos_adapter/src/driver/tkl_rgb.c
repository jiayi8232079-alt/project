...
static bk_err_t tkl_lcd_rgb_gpio_init(DISPLAY_PIXEL_FORMAT_E pixel_fmt)
{
#if CONFIG_SOC_BK7236XX
    if (pixel_fmt == TY_PIXEL_FMT_RGB666) {
        RGB_IO_FUNCTION_ENABLE(LCD_RGB_R2_PIN, LCD_RGB_R2_FUNC);
    } else if (pixel_fmt == TY_PIXEL_FMT_RGB888) {
...
#if CONFIG_SOC_BK7236XX
    if (pixel_fmt == TY_PIXEL_FMT_RGB888) {
...
OPERATE_RET tkl_rgb_init(TUYA_RGB_BASE_CFG_T *cfg)
{
    ...
    if(cfg->pixel_fmt == TY_PIXEL_FMT_RGB565) 
        bk_device.src_fmt = bk_device.out_fmt = PIXEL_FMT_RGB565;
    if(cfg->pixel_fmt == TY_PIXEL_FMT_RGB666) 
        bk_device.src_fmt = bk_device.out_fmt = PIXEL_FMT_RGB666;
    if(cfg->pixel_fmt == TY_PIXEL_FMT_RGB888) {
 ...
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET tkl_rgb_pixel_mode_set(DISPLAY_PIXEL_FORMAT_E mode)
{
    pixel_format_t bk_mode = 0;
    if(mode == TY_PIXEL_FMT_RGB565)
        bk_mode = PIXEL_FMT_RGB565;
    else if(mode == TY_PIXEL_FMT_RGB666)
        bk_mode = PIXEL_FMT_RGB666;
    else if(mode == TY_PIXEL_FMT_RGB888)
