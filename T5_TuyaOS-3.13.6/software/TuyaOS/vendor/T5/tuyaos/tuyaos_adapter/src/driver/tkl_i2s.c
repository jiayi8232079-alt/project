...
static INT32_T ch1_rx_data_handle_cb(uint32_t size)
{
    int ret = 0;
    ret = tkl_semaphore_post(m_semaphore[TUYA_I2S_NUM_1]);
    if (ret != BK_OK) {
...
static INT32_T ch2_rx_data_handle_cb(uint32_t size)
{
    int ret = 0;
    ret = tkl_semaphore_post(m_semaphore[TUYA_I2S_NUM_2]);
    if (ret != BK_OK) {
...
static inline OPERATE_RET i2s_queue_push_latest(TKL_QUEUE_HANDLE q, VOID_T **pdata) {
        ...
        for (size_t i = 0; i < TUYA_I2S_NUM_MAX; i++) {
            ...
            if (m_i2s_config[i].i2s_dma_flags && (m_i2s_config[i].mode & TUYA_I2S_MODE_RX)) {
                ...
                while (loop_guard-- > 0 && ring_buffer_get_fill_size(m_ringbuffer[i]) >= frame_size) {
                    VOID_T *data = tkl_system_psram_malloc(frame_size);
                    if (!data) {
                        break;
                    }
