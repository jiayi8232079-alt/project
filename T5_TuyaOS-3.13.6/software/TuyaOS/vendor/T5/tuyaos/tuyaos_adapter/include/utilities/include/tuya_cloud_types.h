...
typedef enum {
    RGB_DATA_IN_FALLING_EDGE = 0,
    RGB_DATA_IN_RISING_EDGE,
} RGB_DATA_CLK_EDGE_E;

typedef struct {
    UINT32_T clk;
    RGB_DATA_CLK_EDGE_E out_data_clk_edge;  /** rgb data output in clk rising or falling */
    UINT16_T width;
    UINT16_T height;
...
typedef OPERATE_RET (*DVP_SET_UP_CB)(VOID_T *args);
typedef TUYA_DVP_FRAME_MANAGE_T *(*DVP_FRAME_ASSIGN_CB)(TUYA_FRAME_FMT_E fmt, VOID_T *args);
typedef OPERATE_RET (*DVP_FRAME_POST_CB)(TUYA_DVP_FRAME_MANAGE_T *dvp_frame, VOID_T *args);
