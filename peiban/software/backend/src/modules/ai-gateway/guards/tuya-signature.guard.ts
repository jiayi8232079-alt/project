...
export class TuyaSignatureGuard implements CanActivate {
  ...
  canActivate(execCtx: ExecutionContext): boolean {
    if (!this.enabled) {
      if (this.allowUnsignedDev) return true;
      throw new UnauthorizedException('AI_GATEWAY_HMAC_SECRET 未配置');
    }
